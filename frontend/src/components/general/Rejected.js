...
function RejectedClaims() {
    ...
    useEffect(() => {
        const fetchClaims = async () => {
            try {
                ...
                const rejectedClaims = response.data.filter(claim => claim.claimStatus === 'Rejected');
    ...
    const getCircleColor = (status) => {
        return status === 'Rejected' ? '#f44336' : '#00bcd4'; 
    };
    ...
    return (
        ...
        <div id="rejected-claims">
            <h2 id="claims-header">Rejected Claims</h2>
            {error && <p id="error-message">{error}</p>}
            {claims.length === 0 ? (
                <p id="no-claims">No rejected claims found.</p>
            ) : (
                <table>
                    <thead>
                        <tr>
