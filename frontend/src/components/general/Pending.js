...
function PendingClaims() {
    ...
    useEffect(() => {
        const fetchClaims = async () => {
            try {
                ...
                const pendingClaims = response.data.filter(claim => claim.claimStatus === 'Pending');
    ...
    const getCircleColor = (status) => {
        return status === 'Pending' ? '#ff9800' : '#00bcd4';
    };
    ...
    return (
        ...
        <div id="pending-claims">
            <h2 id="claims-header">Pending Claims</h2>
            {error && <p id="error-message">{error}</p>}
            {claims.length === 0 ? (
                <p id="no-claims">No pending claims found.</p>
            ) : (
                <table>
                    <thead>
                        <tr>
                            <th className='ok' onClick={() => requestSort('id')}>Claim ID
                            <img className="sort-icon" src="sort.png" alt="Sort" />
                            </th>
                            <th className='ok' onClick={() => requestSort('user.username')}>Claimant Name
                            <img className="sort-icon" src="sort.png" alt="Sort" />
                            </th>
                            <th className='ok' onClick={() => requestSort('claimType')}>Claim Type
                            <img className="sort-icon" src="sort.png" alt="Sort" />
                            </th>
                            <th className='ok' onClick={() => requestSort('claimStatus')}>Status</th>
                            <th className='ok' onClick={() => requestSort('submissionDate')}>Submission Date
                            <img className="sort-icon" src="sort.png" alt="Sort" />
                            </th>
